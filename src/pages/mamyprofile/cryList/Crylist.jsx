...
//       const totalPages = Math.ceil(AllCries.length / contentperpage);
// const indexOfLastItem = currentPage * contentperpage;
// const indexOfFirstItem = indexOfLastItem - contentperpage;
// const currentItems = AllCries.slice(indexOfFirstItem, indexOfLastItem);
...
export default function CryList() {
  ...
  const finalCries = sortCries(filtered);
  const searchedCries = search.trim()
    ? finalCries.filter((cry) =>
        cry.class.toLowerCase().includes(search.trim().toLowerCase())
      )
    : finalCries;

  // pagination logic
  const totalPages = Math.ceil(searchedCries.length / contentperpage);
  const indexOfLastItem = currentPage * contentperpage;
  const indexOfFirstItem = indexOfLastItem - contentperpage;
  const currentItems = searchedCries.slice(indexOfFirstItem, indexOfLastItem);

  const cardscry = currentItems.map((cry, index) => {
    const time = new Date(cry.createdAt).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
    return (
      <div key={cry._id} className="crycard">
        <div className="cryandimg">
          <img src={getCryImage(cry.class)} alt={cry.class} className="cry-icon" />
          <div className="crydetalis">
            <p>
              Crying for {durations[cry._id] ? durations[cry._id].toFixed(1) : "..."} sec
            </p>
            <span>{time}</span>
          </div>
        </div>
            ...
            <div className="search">
              <input
                ...
                onChange={(e) => {
                  ...
                  setCurrentPage(1); // reset page عند البحث
                }}
              />
              <span>
                <CiSearch />
          ...
          {loading ? (
            <>
              <div className="cryloader">
                ...
                <div className="othersideloader"></div>
              </div>
              <div className="cryloader">
                <div className="sideoneloader">
                  <div className="img"></div>
  ...
  <div className="pagination">
    ...
    {currentPage > 2 && (
      <>
        ...
        {currentPage > 3 && <span className="dots" style={{color:"pink"}}>...</span>}
      </>
    )}
    ...
    {currentPage < totalPages - 1 && (
      <>
        {currentPage < totalPages - 2 && <span className="dots" style={{color:"pink"}}>...</span>}
        <button
